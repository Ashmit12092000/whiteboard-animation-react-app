...
import { BOARD_TYPES, HAND_OPTIONS, resolveHandOption } from '../../assets';
...
export function ProjectSettingsModal() {
  ...
  // "Done" opens customizer; customizer Save commits everything then closes both
  const handleDone = () => setShowHandCustomizer(true);
  ...
  const handleCustomizerSave = (cfg) => {
    ...
    updateProjectSettings(finalConfig);
    setShowHandCustomizer(false);
    closeProjectSettings();
  };

  const handleClose = () => closeProjectSettings();
      ...
      >
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={handleClose} style={styles.btnSec}>Cancel</button>
            <button onClick={handleDone} style={styles.btnDone}>Customize Hand →</button>
          </div>
        </div>
      </div>
...
function HandCustomizerModal({ handId, customHands, initialConfig, onSave, onBack }) {
  ...
  const imgSrc  = handOpt.isCustom ? handOpt.src : (handOpt.isSvg ? `${PUB}/hand-pencil.svg` : `${PUB}${handOpt.src}`);
...
function HandThumb({ opt }) {
  ...
  const imgSrc = opt.isCustom ? opt.src : (opt.isSvg ? `${PUB}/hand-pencil.svg` : `${PUB}${opt.src}`);
...
const styles = {
  ...
  btnDone: {
    flex: 2, padding: '9px 0', background: '#0f3460', border: '1.5px solid #1e3a8a',
    borderRadius: 8, color: '#93c5fd', cursor: 'pointer', fontWeight: 700, fontSize: 14,
  },
};