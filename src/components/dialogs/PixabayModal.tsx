...
const IMAGE_FILTERS = [
  ...
  { id: 'vintage',   label: 'Vintage',   css: 'sepia(40%) contrast(110%) brightness(95%) saturate(80%)' },
  { id: 'warm',      label: 'Warm',      css: 'saturate(150%) brightness(105%) hue-rotate(-15deg)' },
  { id: 'cool',      label: 'Cool',      css: 'saturate(110%) brightness(105%) hue-rotate(30deg)' },
  ...
  { id: 'noir',      label: 'Noir',      css: 'grayscale(100%) contrast(150%) brightness(85%)' },
  { id: 'dreamy',    label: 'Dreamy',    css: 'brightness(110%) saturate(120%) contrast(90%) blur(0.5px)' },
];
...
export default function PixabayModal({ onClose }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      drawEditor(canvas, ctx, img, rotation, cropBox, activeFilter);
    };
    img.src = selected.src;
  }, [selected, rotation, cropBox, activeFilter]);
  ...
  const addToCanvas = async () => {
    ...
    const finalDataUrl = await buildFinalImage(canvas, imageRef.current, rotation, cropBox, activeFilter);
          ...
          {selected && (
            <div style={{
              ...
              <div style={{ padding: '10px 14px', borderTop: '1px solid #1e293b', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {/* Preset Filters */}
                <div>
                  <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 8 }}>Filters</div>
                    ...
                    {IMAGE_FILTERS.map(f => (
                      ...
                      >
                        <div style={{
                          ...
                          border: `2px solid ${activeFilter === f.id ? '#3b82f6' : '#334155'}`,
                          boxShadow: activeFilter === f.id ? '0 0 0 2px #3b82f640' : 'none',
                          transition: 'border-color 0.15s',
                        }}>
                          <img
                            src={selected?.src}
                            alt={f.label}
                            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', filter: f.css }}
                          />
                        </div>
                        <div style={{ fontSize: 9, color: activeFilter === f.id ? '#3b82f6' : '#64748b', marginTop: 3, fontWeight: activeFilter === f.id ? 700 : 400 }}>
                          {f.label}
                        </div>
                      </div>
                    ))}
...
// ── Drawing helpers ───────────────────────────────────────────────────────────
function drawEditor(canvas, ctx, img, rotation, cropBox, activeFilter = 'none') {
  ...
  // Reset filter before drawing crop UI
  ctx.filter = 'none';
...
async function buildFinalImage(editorCanvas, img, rotation, cropBox, activeFilter = 'none') {
  const { dw, dh } = fitInBox(img.naturalWidth, img.naturalHeight, EDITOR_W, EDITOR_H);
  ...
  const filterDef = IMAGE_FILTERS.find(f => f.id === activeFilter);
  tctx.filter = filterDef ? filterDef.css : 'none';
