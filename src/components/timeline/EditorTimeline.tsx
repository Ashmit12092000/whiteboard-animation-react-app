...
// ─── Audio sub-track rows (voice / music / tts) ────────────────────────────────
const AUDIO_ROW_H = 34;
const AUDIO_ROW_TYPES = [
  { type: 'voice', label: '🎙 Voice', color: '#b45309', match: t => (t.type ?? 'voice') === 'voice' },
...
function AudioTrackContextMenu({ x, y, track, onClose, onTrim }) {
  ...
  const items = [
    ...
    { label: 'Volume 75%',  icon: '🔉', action: () => { update({ volume: 0.75 }); onClose(); } },
    { label: 'Volume 50%',  icon: '🔉', action: () => { update({ volume: 0.5 }); onClose(); } },
    { label: 'Volume 25%',  icon: '🔈', action: () => { update({ volume: 0.25 }); onClose(); } },
    { sep: true },
    // Fade
    { label: fadeIn  > 0 ? 'Remove Fade In'  : 'Fade In (0.5s)',  icon: '↗', action: () => { update({ fadeIn:  fadeIn  > 0 ? 0 : 0.5 }); onClose(); } },
    { label: fadeOut > 0 ? 'Remove Fade Out' : 'Fade Out (0.5s)', icon: '↘', action: () => { update({ fadeOut: fadeOut > 0 ? 0 : 0.5 }); onClose(); } },
    { sep: true },
    // Loop (voice only — TTS is regenerated each time)
    ...(!isTTS ? [{ label: loop ? 'Disable Loop' : 'Enable Loop', icon: '🔁', action: () => { update({ loop: !loop }); onClose(); } }, { sep: true }] : []),
...
// ─── Playhead needle ──────────────────────────────────────────────────────────
function Playhead({ playheadX, trackCount, cameraRowH, audioRowsH, onScrubStart }) {
  const totalH = HEADER_H + (trackCount * TRACK_H) + (cameraRowH ?? 34) + (audioRowsH ?? 34);
  return (
    <div
...
export default function EditorTimeline() {
  ...
  // ── Voice recorder / TTS modals (state lives in store — toggled from canvas HUD) ──
  const showVoiceRecorder    = useStore(s => s.showVoiceRecorder);
  const setShowVoiceRecorder = useStore(s => s.setShowVoiceRecorder);
  const showTTSModal         = useStore(s => s.showTTSModal);
  const setShowTTSModal      = useStore(s => s.setShowTTSModal);
                ...
                <>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
                    ...
                    <CameraEasingPreview
                      ...
                      onChange={name => { commitHistory(); updateCameraKeyframe(selectedSceneId, selectedKf.id, { easing: name }); }}
                    />
                  </div>
              ...
              >
                ...
                <div style={{ minWidth: totalDurationS * pxPerS + 32, position: 'relative' }}>
                  ...
                  <Playhead
                    ...
                    cameraRowH={34}
                    audioRowsH={AUDIO_ROW_H * AUDIO_ROW_TYPES.filter(({ match }) => audioTracks.some(match)).length}
