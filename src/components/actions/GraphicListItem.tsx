...
import { useStore, MIN_GRAPHIC_DURATION } from '../../store';
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div
          ...
          onDragEnd={onDragEnd}
          style={{ color: '#475569', fontSize: 14, cursor: 'grab', flexShrink: 0, lineHeight: 1 }}
        >
          ⠿
        </div>
        ...
        >
          ...
          <Row label="Duration (s)">
            <NumInput
              ...
              min={0.1} step={0.5}
              onChange={v => setGraphicDuration(graphic.id, v)}
            />
          </Row>

          {/* Hand Speed — adjusting this rescales Duration live, same as Doodly */}
          <Row label="Hand Speed">
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              <input
                ...
                onChange={e => {
                  const newHs = Number(e.target.value);
                  const hs    = graphic.hand_speed ?? 1.0;
                  // base_duration = the duration at 1× hand speed; preserved across
                  // slider moves so duration scales smoothly and reversibly.
                  const base  = graphic.base_duration ?? (graphic.duration * hs);
                  const newDuration = Math.max(MIN_GRAPHIC_DURATION, base / newHs);
                  updateGraphicProps(graphic.id, {
                    hand_speed: newHs,
                    base_duration: base,
