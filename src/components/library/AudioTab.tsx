...
// ─── Waveform visualiser ───────────────────────────────────────────────────────
function Waveform({ audioBuffer, trimStart, trimEnd, duration, onTrimChange, playing, playhead, onCommitHistory }) {
...
// ─── Single track row ──────────────────────────────────────────────────────────
function AudioTrack({ track, isSelected, onSelect, onDelete, onUpdate, onCommitHistory }) {
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <input
          ...
          onMouseDown={e => { e.stopPropagation(); onCommitHistory?.(); }}
          onTouchStart={e => { e.stopPropagation(); onCommitHistory?.(); }}
      ...
      {isSelected && (
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          ...
          {/* Loop toggle */}
          <button
            onClick={e => { e.stopPropagation(); onCommitHistory?.(); onUpdate({ loop: !track.loop }); }}
      ...
      {/* ── Trim panel ── */}
      {isSelected && showTrim && (
        <TrimPanel track={track} onUpdate={onUpdate} onCommitHistory={onCommitHistory} />
      )}

      {/* ── Filter panel ── */}
      {isSelected && showFilters && (
        <FilterPanel track={track} onUpdate={onUpdate} onCommitHistory={onCommitHistory} />
      )}
    </div>
  );
}
...
// ─── Trim panel ───────────────────────────────────────────────────────────────
function TrimPanel({ track, onUpdate, onCommitHistory }) {
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        <input
          ...
          onMouseDown={() => onCommitHistory?.()}
          onTouchStart={() => onCommitHistory?.()}
...
// ─── Filter panel ─────────────────────────────────────────────────────────────
function FilterPanel({ track, onUpdate, onCommitHistory }) {
  return (
    <div style={{
      marginTop: 10, padding: '10px 12px', background: C.surface2,
      borderRadius: 8, border: `1px solid ${C.border}`,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 5 }}>
        {AUDIO_FILTERS.map(f => (
          <button
            key={f.id}
            onClick={() => { onCommitHistory?.(); onUpdate({ filter: f.id }); }}
            title={f.desc}
            style={{
              padding: '7px 8px',
