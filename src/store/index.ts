...
export const useStore = create((set, get) => ({
  ...
  // ── Active editor state ──────────────────────────────────────────────────────
  project: null,
  lastSavedSnapshot: null, // JSON string of project at last save/load — used to detect unsaved changes
  ...
  createNewProject(title, boardType, canvasSizeKey) {
    ...
    set({
      ...
      lastSavedSnapshot: JSON.stringify(p),
  ...
  saveProject() {
    ...
    persistProjects(updated);
    set({ recentProjects: updated, project: saved, lastSavedSnapshot: JSON.stringify(saved) });
  ...
  closeProject() {
    set({ view: 'launch', project: null, lastSavedSnapshot: null, selectedSceneId: null, selectedGraphicId: null });
  },
  ...
  // ── Undo / Redo ──────────────────────────────────────────────────────────────
  // Records the CURRENT project state as an undo entry, without changing
  // anything else. Used by continuous/drag-style edits (move, resize, rotate,
  // camera-keyframe drag, etc.) which call this once — right before the first
  // change of the gesture — so the whole drag becomes a single undo step
  // instead of one entry per pointermove.
  commitHistory() {
  ...
  setShowVoiceRecorder(v) { set({ showVoiceRecorder: v }); },
  setShowTTSModal(v)      { set({ showTTSModal: v }); },
